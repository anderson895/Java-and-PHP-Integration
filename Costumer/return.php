...
if (isset($_POST['btnPayment'])) {
   ...
   $returnData = $_POST['orderId'] ."," .$payment.",".$current_time.",".$w3review."\n";
...
if (file_exists($returnFile)) {
    $returnContent = file_get_contents($returnFile);
    $returnLines = explode("\n", $returnContent);
    $orderIdFound = false;

    foreach ($returnLines as $returnLine) {
        $returnColumns = explode(",", $returnLine);
        if ($returnColumns[0] == $orderId) {
            // Order ID already exists in costumerReturn.txt
            echo "Order ID already exists in costumerReturn.txt";
			   
			// Update the existing order data
			// Read the contents of the file into an array
			$orders = file($returnFile, FILE_IGNORE_NEW_LINES);

			// Loop through each order to find a matching order ID
			foreach ($orders as $index => $order) {
				$orderData = explode(',', $order);
				$currentOrderID = trim($orderData[0]); // Trim any whitespace from the order ID
				if ($currentOrderID == $orderId) {
					// Update the current line with the new data
					$orders[$index] = $returnData;

					// Save the updated contents back to the file
					file_put_contents($returnFile, implode("\n", $orders));

					// Exit the loop since we've found and updated the order
					break;
				}
			}
			
			// Set the flag to indicate that the order ID was found
			$orderIdFound = true;

			// Add your code here for what to do if the order ID already exists
			break;
		}
	}

	// If the order ID wasn't found, append the new order data to the file
	if (!$orderIdFound) {
		echo "Append Successfully";
		file_put_contents($returnFile, $returnData, FILE_APPEND);

		// Add your code here for what to do if the order ID doesn't exist
	}

} else {
	// File does not exist, create a new file and append the order data
	echo "Append Successfully";
	file_put_contents($returnFile, $returnData, FILE_APPEND);

	// Add your code here for what to do if the file doesn't exist
}
...
<p><label for="w3review">why are you returning the item?</label></p>
